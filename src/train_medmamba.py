...
# Add MedMamba to path
sys.path.append(os.path.join(os.path.dirname(__file__), '../../MedMamba'))
...
def get_model(model_size, num_classes, device):
    ...
    if model_size == 'tiny':
        return VSSM(depths=[2, 2, 4, 2], dims=[96,192,384,768], num_classes=num_classes).to(device)
    elif model_size == 'small':
        return VSSM(depths=[2, 2, 8, 2], dims=[96,192,384,768], num_classes=num_classes).to(device)
    else:
        return VSSM(depths=[2, 2, 12, 2], dims=[128,256,512,1024], num_classes=num_classes).to(device)
...
def train(args):
    ...
    optimizer = AdamW(
        ...
        betas=(0.9, 0.999),  # Same as AdamW defaults
        weight_decay=args.weight_decay
    )
    ...
    for epoch in range(args.epochs):
        ...
        if task_type == 'binary':
            ...
            wandb_metrics = {
                ...
                'learning_rate': args.learning_rate  # Use const leraning rate
            }
